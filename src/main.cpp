...
#if defined(NDEBUG)
# error "TLGCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x117c0ec1c77e848f00496eb3890c96e1eaf444d5a91a035e30a675ced5add27d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TLGCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TLGCoin Signed Message:\n";
...
{
    // TLGCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TLGCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TLGCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1505160913, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Exchange controls relaxed for IP foreign investment";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0493ccdfa46d96cbdc28616751233dc5fa803a8936a097932b60a796727c23a8c1484aede05f542111f60726c93910924eb93060806435fa34b896c32a69e41afc") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1505160913;
        ...
        assert(block.hashMerkleRoot == uint256("0xca8295f5bdb1caca6aa8f168f5f7a4d6424d7f90bbdedc3d06a1d7af1d9bb930"));
...
unsigned char pchMessageStart[4] = { 0xec, 0x02, 0x7a, 0x78 }; // TLGCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TLGCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TLGCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TLGCoinMiner(CWallet *pwallet)
{
    printf("TLGCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tlgcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TLGCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TLGCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TLGCoinMiner, pwallet));
}
